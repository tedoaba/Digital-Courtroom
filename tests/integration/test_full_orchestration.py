...
def test_full_swarm_execution(mock_initial_state, mocker):
    ...
    repo_mock = mocker.patch("src.nodes.detectives.repo_investigator", return_value={
        "evidences": {"repo": [
            Evidence(
                evidence_id="repo_1", source="repo", evidence_class=EvidenceClass.GIT_FORENSIC,
                goal="typing", found=True, content="typed", location="main.py",
                rationale="found types", confidence=1.0, timestamp=datetime.now()
            )
        ]}
    })
    repo_mock.__name__ = "repo_investigator"

    doc_mock = mocker.patch("src.nodes.detectives.doc_analyst", return_value={"evidences": {"docs": []}})
    doc_mock.__name__ = "doc_analyst"

    vis_mock = mocker.patch("src.nodes.detectives.vision_inspector", return_value={"evidences": {"vision": []}})
    vis_mock.__name__ = "vision_inspector"
    
    judge_mock = mocker.patch("src.nodes.judges.evaluate_criterion", return_value={
        "opinions": [
            JudicialOpinion(
                opinion_id="judge_1", judge="TechLead", criterion_id="typing",
                score=5, argument="Excellent", cited_evidence=["repo_1"]
            )
        ]
    })
