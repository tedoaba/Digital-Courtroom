import pytest
from src.nodes.justice import round_half_up, round_score
...
def test_synthesize_criterion_high_variance():
    ...
    op1 = JudicialOpinion(
        opinion_id="p1", judge="Prosecutor", criterion_id="crit1", 
        score=1, argument="Very Bad", cited_evidence=[]
    )
    op2 = JudicialOpinion(
        opinion_id="d1", judge="Defense", criterion_id="crit1", 
        score=5, argument="Very Good", cited_evidence=[]
    )
    op3 = JudicialOpinion(
        opinion_id="t1", judge="TechLead", criterion_id="crit1", 
        score=2, argument="Bad", cited_evidence=[]
...
def test_security_override_prosecutor_trigger():
    ...
    op1 = JudicialOpinion(
        opinion_id="p1", judge="Prosecutor", criterion_id="crit1", 
        score=1, argument="Potential shell injection detected.", 
        cited_evidence=[], charges=["Security Violation: shell injection"]
    )
    op2 = JudicialOpinion(
        opinion_id="d1", judge="Defense", criterion_id="crit1", 
        score=5, argument="It's fine.", cited_evidence=[]
    )
    op3 = JudicialOpinion(
        opinion_id="t1", judge="TechLead", criterion_id="crit1", 
        score=5, argument="Works.", cited_evidence=[]
    )
...
def test_security_override_evidence_trigger():
    ...
    op1 = JudicialOpinion(
        opinion_id="p1", judge="Prosecutor", criterion_id="crit1", 
        score=5, argument="Looks good", cited_evidence=[]
    )
    op2 = JudicialOpinion(
        opinion_id="d1", judge="Defense", criterion_id="crit1", 
        score=5, argument="Perfect", cited_evidence=[]
    )
    op3 = JudicialOpinion(
        opinion_id="t1", judge="TechLead", criterion_id="crit1", 
        score=5, argument="Solid", cited_evidence=[]
    )
    ...
    sec_evidence = Evidence(
        evidence_id="repo_sec_01", source="repo", 
        evidence_class=EvidenceClass.SECURITY_VIOLATION,
        goal="check security", found=True, location="test.py",
        rationale="os.system used", confidence=1.0, 
        timestamp=datetime.now(), content="os.system('rm -rf /')"
    )
...
def test_fact_supremacy_penalty():
    ...
    op2 = JudicialOpinion(
        opinion_id="d1", judge="Defense", criterion_id="crit1", 
        score=5, argument="It's fine because of X.", cited_evidence=["missing_01"]
    )
    op3 = JudicialOpinion(
        opinion_id="t1", judge="TechLead", criterion_id="crit1", 
        score=3, argument="Mid", cited_evidence=[]
