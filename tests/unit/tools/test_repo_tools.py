...
def test_clone_repository_timeout(mocker, tmp_path):
    mocker.patch("subprocess.run", side_effect=subprocess.TimeoutExpired(cmd="git", timeout=60))
...
def test_clone_repository_failure(mocker, tmp_path):
    mocker.patch("subprocess.run", side_effect=subprocess.CalledProcessError(returncode=1, cmd="git", stderr="Not found"))
