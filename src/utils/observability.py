...
class DashboardManager:
    ...
    def generate_layout(self) -> Layout:
        ...
        layout["header"].update(Panel("Digital Courtroom — Ironclad Swarm Monitor", style="bold white on blue"))
        ...
        main_table.add_row("Status", "PAUSED" if self.status.is_paused else "RUNNING", style="bold yellow" if self.status.is_paused else "bold green")
        main_table.add_row("Open Breakers", ", ".join(self.status.active_circuit_breakers) or "None")
        
        layout["main"].update(Panel(main_table, title="Swarm State"))
        layout["footer"].update(Panel("[p] Pause | [r] Resume | [c] Reset CB", style="dim italic"))
        
        return layout

    def start(self):
        self._running = True
        self.live = Live(self.generate_layout(), console=self.console, refresh_per_second=1)
        self.live.start()
