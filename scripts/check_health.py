...
def run_checks():
    ...
    cons_table.add_row(
        "Fan-out Detectives",
        "Verified" if "repo_investigator" in analysis.get("nodes", []) else "FAILED",
    )
    ...
    if analysis["violations"]:
        for v in analysis["violations"]:
            cons_table.add_row("[red]Violation[/red]", v)
    ...
    env_table.add_row(
        ...
        "[green]FOUND[/green]" if pre_commit else "[red]MISSING[/red]",
    )
